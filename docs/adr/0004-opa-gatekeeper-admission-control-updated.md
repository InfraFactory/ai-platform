...
### Critical: Audit loop requires explicit sync configuration

Gatekeeper's audit loop does **not** evaluate existing cluster resources by default.
Without a `sync.syncOnly` block in the Config CR, the audit pod has no resource cache
to evaluate — violations only surface at admission time for newly created resources,
not against resources that pre-existed the policy.
...
Keep `syncOnly` narrow — every resource type added to the sync list increases memory
consumption on the audit pod. Only add resource types that existing policies actually
evaluate. The current four policies evaluate Pods and Services only; Namespace is
included for future exemption-label policies.

### Critical: Namespace exemptions in the Config CR (not HelmRelease values)

Namespace exemptions are configured in the Gatekeeper `Config` CR, not in the
HelmRelease `values`. The Helm chart does not expose a simple `exemptNamespaces`
list — the Config CR is the correct mechanism.
...
- [x] `flux-system` added to `excludedNamespaces` in Gatekeeper Config CR
- [x] `dependsOn` added to the policies Kustomization referencing `opa-gatekeeper`
- [x] `sync.syncOnly` configured in Config CR for Pod, Service, Namespace
- [x] Four policies deployed and validated: `require-resource-limits`, `deny-public-loadbalancer`, `restrict-model-namespace`, `require-sa-token-control`
